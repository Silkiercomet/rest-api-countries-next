...
const SearchBar = ({allCountries,setDisplayed,setPageNumber}) => {
  ...
  const filterByRegions = (region) => {
    ...
    useEffect(() => {
      if(input !== ""){
          let count = allCountries.filter(c => c.name.common.toLowerCase().slice(0,input.length).includes(input.toLocaleLowerCase()))
          setPageNumber(1)
          console.log(count)
  ...
  return (
    ...
    <li className={ `light-bg-element ${styles.menu__select}`}>
      <select name="Filter by Region" ref={selectRef} className={'light-bg-element light'} onChange={(e) => {setDisplay(regions[e.target.value]?.())
        setRender(!render)}} >
